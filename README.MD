...
As I write this, latest `springaddons.version` is `2.5.4` but I could forget to update before releasing, so please refer to https://repo1.maven.org/maven2/com/c4-soft/springaddons/spring-addons/ to pick latest available release of one of the following:
...
Currently limited to a portable Authentication implementation for OpenID: `OidcIdAuthenticationToken`, which is intended to be used with any OpenID authorization server (Keycoak, Auth0, MS Identity Server, ...).
