...
Sample for `@WithMockJwtAuth` `spring-addons-oauth2-test`:
```java
...
Sample for `@OpenId` with `spring-addons-webmvc-jwt-test` (but others are very similar):
```java
@WebMvcTest(GreetingController.class)
...
- support multiple OIDC authorization-server `issuer` URI (can configure as as many as you need for multi-tenant scenarios)
...
### What is auto-configured with resource-server starters
...
- one main @Bean is configured: `SecurityFilterChain` for servlets and `SecurityWebFilterChain` for reactive apps.
