...
public class KeycloakAuthenticationTokenTestingBuilder<T extends KeycloakAuthenticationTokenTestingBuilder<T>> extends KeycloakAuthenticationTokenBuilder<T> {
	...
	public T authorities(Stream<String> authorities) {
		final Set<String> authoritiesSet = authorities.collect(Collectors.toSet());
	...
	public KeycloakAuthenticationToken build() {
		...
		final KeycloakPrincipal<RefreshableKeycloakSecurityContext> principal = new KeycloakPrincipal<>(accessToken.getPreferredUsername(), securityContext);
