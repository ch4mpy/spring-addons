...
| Building the `Authentication` | `jwtAuthenticationConverter` (a `JwtAbstractAuthenticationTokenConverter`) | A `JwtAuthenticationConverter` holding the converter above and setting the principal name from the right claim. Exposing it as a bean rather than a lambda inside the chain is also what makes `@WithJwt` able to run it in tests. |
