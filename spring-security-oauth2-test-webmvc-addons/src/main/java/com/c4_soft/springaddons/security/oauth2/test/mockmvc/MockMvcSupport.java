 ...
 * <ul>
 * <li>auto sets "Accept" and "Content-Type" headers according {@code com.c4-soft.springaddons.test.web.default-media-type}
 * and {@code com.c4-soft.springaddons.test.web.default-charset} to test properties, defaulted to {@code application/json} and {@code utf-8}</li>
 * <li>serializes request body according to Content-type using <b>registered message converters</b></li>
 * <li>provides with shortcuts to issue requests in basic but most common cases (no fancy headers, cookies, etc):
 * get, post, patch, put and delete  methods</li>
 * <li>wraps MockMvc {@link MockMvc#perform(RequestBuilder) perform} and exposes request builder helpers for advanced cases
 * (when you need to further customize {@link MockHttpServletRequestBuilder} with cookies or additional headers for instance)</li>
 * </ul>
 *
...
public class MockMvcSupport {
	 ...
	 *                            message converters
	 * @param defaultMediaType    media-type to be used ({@code Content-type} or
	 *                            {@code Accept} headers, and payload
	 *                            serialization), when not specified as argument of
	 *                            this helper methods.<br>
	 *                            Set with
	 *                            {@code com.c4-soft.springaddons.test.web.default-media-type}
	 ...
	 * MockHttpServletRequestBuilder} with relevant "Accept" and "Content-Type"
	 * headers. You might prefer to use
	 * {@link #getRequestBuilder(MediaType, String, Object...) getRequestBuilder} or
	 * alike which go further with request pre-configuration or even
	 * {@link #get(MediaType, String, Object...) get},
	 ...
	 * submit it (Accept header defaulted to what this helper was constructed with).
	 *
	 * @param urlTemplate API endpoint to be requested
	 * @param uriVars     values to replace endpoint placeholders with
	 * @return API response to test
	 ...
	 * this class
	 * {@link org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder
	 * MockHttpServletRequestBuilder} factories instead (getRequestBuilder,
	 ...
	 * @throws Exception if things go wrong (no registered serializer for payload
	 *                   type and asked MediaType, serialization failure, ...)
	 */
	public <T> MockHttpServletRequestBuilder
