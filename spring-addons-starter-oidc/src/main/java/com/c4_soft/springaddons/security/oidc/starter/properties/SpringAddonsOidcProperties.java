 ...
 * </ul>
 * <b>spring.security.oauth2.resourceserver.jwt.* properties are ignored.</b> The reason for that is
...
public class SpringAddonsOidcProperties {
   ...
   * configuration for each issuer. A minimum of one issuer is required. <b>Properties defined here
   * are a replacement for spring.security.oauth2.resourceserver.jwt.*</b> (which will be ignored).
   ...
   * dependencies to be defined. <b>Properties defined here are a complement for
   * spring.security.oauth2.client.*</b> (which are required when enabling spring-addons client
   * filter-chain).
   */
