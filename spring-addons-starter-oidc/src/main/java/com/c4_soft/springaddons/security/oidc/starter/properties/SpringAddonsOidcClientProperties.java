...
public class SpringAddonsOidcClientProperties {
	...
	/**
	 * Path matchers for the routes secured with the auto-configured client filter-chain. If left empty, OAuth2 client auto-configuration is
	 * disabled. It should include "/login/**" and "/oauth2/**" for login process. Can be set to "/**" to intercept all requests (OAuth2 client
	...
	/**
	 * HTTP status for redirections in OAuth2 login and logout. You might set this to something in 2xx range (like OK, ACCEPTED, NO_CONTENT,
	 * ...) for single page and mobile applications to handle this redirection as it wishes (change the user-agent, clear some headers, ...).
	 */
	 ...
	 * <a href= "https://openid.net/specs/openid-connect-rpinitiated-1_0.html">RP-Initiated Logout</a> standard, but exposing a logout end-point
	 * expecting an authorized GET request with following request params:
	 * <ul>
