 ...
 * "com.c4-soft.springaddons.oidc.client.security-matchers" property has at least one entry. If defined, it is with highest precedence, to
 ...
 * <li>logoutSuccessHandler: a {@link LogoutSuccessHandler}. Default instance is a {@link SpringAddonsLogoutSuccessHandler} which logs a
...
public class SpringAddonsOidcClientWithLoginBeans {
	 ...
	 * generated one is used (be aware that it does not work when bound to 80 or 8080 with SSL enabled, so, in that case, use another port or
	 * define a login path and a controller to handle it)</li>
	 ...
	 * @param  logoutSuccessHandler                 Defaulted to {@link SpringAddonsLogoutSuccessHandler} which can handle "almost" RP Initiated
	 *                                              Logout conformant OPs (like Auth0 and Cognito). Default is a
	 *                                              {@link SpringAddonsLogoutSuccessHandler}
	 ...
	 * @param  oidcBackChannelLogoutHandler         if present, Back-Channel Logout is enabled. A default {@link OidcBackChannelLogoutHandler}
	 *                                              is provided if com.c4-soft.springaddons.oidc.client.back-channel-logout.enabled is true
	...
	SecurityFilterChain springAddonsClientFilterChain(
			...
			Optional<OidcBackChannelLogoutHandler> oidcBackChannelLogoutHandler)
			throws Exception {
		// @formatter:off
	...
	/**
	 * Build logout request for <a href= "https://openid.net/specs/openid-connect-rpinitiated-1_0.html">RP-Initiated Logout</a>. It works with
	 * most OIDC provider: those complying with the spec (Keycloak for instance), off course, but also those which are close enough to it
	 * (Auth0, Cognito, ...)
	 *
	 * @param  addonsProperties {@link SpringAddonsOAuth2ClientProperties} to pick logout configuration for divergence to the standard (logout
	 *                          URI not provided in .well-known/openid-configuration and non-conform parameter names)
	 ...
	 * <a href= "https://openid.net/specs/openid-connect-rpinitiated-1_0.html">RP-Initiated Logout</a> (or approximately complying to it like
	 * Auth0 or Cognito)
	 *
	...
	/**
	 * @return a Post processor for access control in Java configuration which requires users to be authenticated. It is called after
	 *         "permit-all" configuration property was applied.
	 */
	@ConditionalOnMissingBean
	@Bean
	...
	InvalidSessionStrategy invalidSessionStrategy(SpringAddonsOidcProperties addonsProperties) {
		final var location = addonsProperties.getClient().getLoginUri()
				.orElse(
						UriComponentsBuilder.fromUri(addonsProperties.getClient().getClientUri())
								.pathSegment(addonsProperties.getClient().getClientUri().getPath(), "/login").build().toUri())
				.toString();
		log.debug(
				"Invalid session. Returning %d and request authentication at %s"
						.formatted(addonsProperties.getClient().getOauth2Redirections().getInvalidSessionStrategy().value(), location));

		if (addonsProperties.getClient().getOauth2Redirections().getInvalidSessionStrategy() == HttpStatus.FOUND) {
			return new SimpleRedirectInvalidSessionStrategy(location);
		}

		return (HttpServletRequest request, HttpServletResponse response) -> {
			response.setStatus(addonsProperties.getClient().getOauth2Redirections().getInvalidSessionStrategy().value());
			response.setHeader(HttpHeaders.LOCATION, location);
			if (addonsProperties.getClient().getOauth2Redirections().getInvalidSessionStrategy().is4xxClientError()
					|| addonsProperties.getClient().getOauth2Redirections().getInvalidSessionStrategy().is5xxServerError()) {
				response.getOutputStream().write("Invalid session. Please authenticate at %s".formatted(location).getBytes());
			}
