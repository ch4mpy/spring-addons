...
/**
 * An authentication failure handler reading post-login failure URI in session (set by the frontend
 * with a header or request param when initiating the authorization_code flow) and using a
 * {@link SpringAddonsOauth2ServerRedirectStrategy}
 *
 * @author Jerome Wacongne ch4mp&#64;c4-soft.com
 * @see SpringAddonsOidcClientProperties#POST_AUTHENTICATION_FAILURE_URI_SESSION_ATTRIBUTE for
 *      constant used as session attribute keys
 * @see SpringAddonsServerOAuth2AuthorizationRequestResolver which sets the post-login URI session
 *      attribute
 */
@Slf4j
public class SpringAddonsOauth2ServerAuthenticationFailureHandler
  ...
  public SpringAddonsOauth2ServerAuthenticationFailureHandler(
      SpringAddonsOidcProperties addonsProperties) {
    this.defaultRedirectUri =
        addonsProperties.getClient().getLoginErrorRedirectPath().orElse(URI.create("/"));
    this.postAuthorizationFailureStatus =
        addonsProperties.getClient().getOauth2Redirections().getPostAuthorizationFailure();
  }
