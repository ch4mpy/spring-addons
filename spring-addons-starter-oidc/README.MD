...
But whatever we take as *Issuer Identifier* for a Microsoft Entra ID instance, we can't be issued JWT access tokens and have a match of the *Issuer Identifier* between the discovery URI, the OpenID configuration payload and tokens `iss` claim. The candidates for the *Issuer Identifier* from which we can build a discovery URI look as follows:
