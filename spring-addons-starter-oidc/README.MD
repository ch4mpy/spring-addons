...
Microsoft Entra ID looks like an OIDC Provider, but it isn't. [The spec](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfig) states that *"OpenID Providers supporting Discovery MUST make a JSON document available at the path formed by concatenating the string `/.well-known/openid-configuration` to the Issuer."*. Tokens contain an `iss` claim built around `sts.microsoft.com`. Something like `https://sts.windows.net/4f68014f-7f14-4f89-8197-06f0b3ff24d9/`, but when browsing to the discovery endpoint, the value for the `issuer` doesn't match (it is something like `https://login.microsoftonline.com/4f68014f-7f14-4f89-8197-06f0b3ff24d9/v2.0`). This breaks [the tokens validation requirement](https://openid.net/specs/openid-connect-core-1_0.html#IDTokenValidation) that *"The Issuer Identifier for the OpenID Provider (which is typically obtained during Discovery) MUST exactly match the value of the iss (issuer) Claim"*.
