...
- [2. Frequently Asked Questions](#faq)
  ...
  * [Why can't I get things working with Microsoft Entra ID?](#2-11)
- [3. Usage](#usage)
...
On a Spring OAuth2 client with `oauth2Login`, things are a bit more complicated as OpenID auto-configuration relies on the *Issuer Identifier* to be set as `spring.security.oauth2.client.provider.{provider-id}.issuer-uri`. If for some reason the authorization server is not accessible using the *Issuer Identifier* (misconfigured containerized environments) or does not match the OpenID spec (Microsoft), we should leave the `issuer-uri` should be left empty, and all other provider properties manually set instead of relying on OpenID auto-configuration for `authorization`, `token`, `jwk-set`, and `userinfo` endpoints.

### <a name="2-11"/>Why can't I get things working with Microsoft Entra ID?
Microsoft Entra ID looks like an OIDC Provider, but it isn't. [The spec](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfig) states that *"OpenID Providers supporting Discovery MUST make a JSON document available at the path formed by concatenating the string `/.well-known/openid-configuration` to the Issuer."*. Tokens contain an `iss` claim built around `sts.microsoft.com`. Something like `https://sts.windows.net/4f68014f-7f14-4f89-8197-06f0b3ff24d9/`, but when browsing to the matching discovery endpoint, the value for the `issuer` doesn't match (it is something like `https://login.microsoftonline.com/4f68014f-7f14-4f89-8197-06f0b3ff24d9/v2.0`). This breaks [the tokens validation requirement](https://openid.net/specs/openid-connect-core-1_0.html#IDTokenValidation) that *"The Issuer Identifier for the OpenID Provider (which is typically obtained during Discovery) MUST exactly match the value of the iss (issuer) Claim"*.

A workaround is to replace the default JWT decoder with one skipping issuer validation (see [Is `iss` configuration property mandatory?](#2-10)). But this makes your Relying Party voluntarily opt-out of the OpenID specification, and is unsafe. A much better option is to use an actual OpenID Provider like Keycloak, Spring Authorization Server, Auth0, Amazon Cognito, or whatever but Microsoft. In the case where you have to use a Microsoft authorization server because this is where corporate users are, still use a proper OpenID Provider for your system and have it federate Microsoft identities. If the Microsoft authorization server is the sole identity source for your OpenID Provider, this can be transparent for end-users, and your Spring applications can stick to OpenID.
