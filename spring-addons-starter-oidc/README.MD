...
With `spring-addons-starter-oidc`, the CSRF cookie path, name, same-site, and domain can be configured if the `csrf` property is set to `cookie-accessible-from-js`:
```yaml
com:
  c4-soft:
    springaddons:
