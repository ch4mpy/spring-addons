...
public class IntrospectionClaimSetAuthenticationRequestPostProcessorTests
	...
	IntrospectionClaimSetAuthenticationRequestPostProcessor mockCh4mpy() {
		return authentication().name("ch4mpy").authorities("TEST_AUTHORITY");
	}
	...
	public void test() {
		final OAuth2ClaimSetAuthentication<IntrospectionClaimSet> actual =
				(OAuth2ClaimSetAuthentication<IntrospectionClaimSet>) getSecurityContextAuthentication(
						mockCh4mpy().postProcessRequest(new MockHttpServletRequest()));
		...
		assertThat(actual.getClaimSet().getSubject()).isEqualTo("testuserid");
	}
