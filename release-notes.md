...
- Same fix for the default `authoritiesConverter`, which backed off only for a `ClaimSetAuthoritiesConverter` bean: an application bean typed `Converter<Map<String, Object>, Collection<? extends GrantedAuthority>>` left two candidates and, the injection parameter being named `authoritiesConverter`, the default was silently used instead of the application's. The default now backs off for any bean of that type; with several, the `@Primary` one or the one named `authoritiesConverter` is used.
- The unused `AuthenticationConverterMissingCondition` is removed.
- Documented that the reactive `csrfCookieWebFilter` default is conditional on the bean **name** (`WebFilter` is too broad a type to back off on): name yours `csrfCookieWebFilter` to replace it.

#### `spring-addons-starter-oidc-test`
- The mocked `clientRegistrationRepository` of `AddonsWebmvcTestConf` and `AddonsWebfluxTestConf` now backs off for any `ClientRegistrationRepository` (resp. `ReactiveClientRegistrationRepository`) bean. It backed off only for an `InMemory(Reactive)ClientRegistrationRepository`, so a test context importing an application repository of another type ended up with two.
...
### `9.3.1`
- Only one `refresh_token` flow at a time per session. Most authorization servers rotate refresh tokens: the one which was used is revoked as soon as a new one is issued. When a user-agent sends parallel requests while the access token in session is expired, Spring Security fires one `refresh_token` flow per request, only one of them can succeed, and all the other requests are answered with a `401` ([spring-security#15145](https://github.com/spring-projects/spring-security/issues/15145), declined there as "not something the framework can solve generally"). The `RefreshToken(Reactive)OAuth2AuthorizedClientProvider` built by `spring-addons-starter-oidc` is now decorated with a new `SingleRefreshTokenFlow(Reactive)OAuth2AuthorizedClientProvider`: the first request to reach the provider runs the flow and the others wait for its result, so the authorization server sees a single token request and the refresh token is spent exactly once. Requests from other sessions hold other refresh tokens and keep being refreshed in parallel. The result of a flow is also shared for a short while after it completed, which covers the requests which had loaded the authorized client from the session just before the refreshed one was saved there. Configurable with `com.c4-soft.springaddons.oidc.client.single-refresh-token-flow.*`, see the [spring-addons-starter-oidc README](https://ch4mpy.github.io/spring-addons/oidc/client/#concurrent-refresh-token-flows) for details. Set `enabled` to `false` to restore the Spring Security behavior. Note that this de-duplicates flows inside a single JVM: behind a load balancer, use session affinity, or read the README section for hints on writing a cross-instance decorator of your own.

### `9.3.0`
- `spring-addons-starter-oidc` now picks the `(Reactive)OAuth2AuthorizedClientManager` implementation matching the flows declared with `spring.security.oauth2.client.registration.*` properties, instead of always building a `Default(Reactive)OAuth2AuthorizedClientManager`:
  - only `authorization_code` registrations: unchanged, a `Default(Reactive)OAuth2AuthorizedClientManager`
  - no `authorization_code` registration (a resource server consuming another resource server with `client_credentials`, typically): an `AuthorizedClientService(Reactive)OAuth2AuthorizedClientManager`, which needs no request and stores the authorized clients in the `(Reactive)OAuth2AuthorizedClientService`, so that tokens are reused across requests instead of being requested again for each of them
  - both: a new `PerRegistration(Reactive)OAuth2AuthorizedClientManager` delegating to one of the two above, depending on the flow of the registration to authorize
- `spring-addons-starter-oidc` now exposes a `HttpSessionOAuth2AuthorizedClientRepository` (or a `WebSessionServerOAuth2AuthorizedClientRepository` in a reactive application) when at least one registration uses `authorization_code`. This replaces the Spring Boot default, which keeps the authorized clients in a map local to the JVM: in a BFF, tokens now follow the session and are replicated by Spring Session and released when the session is closed.
- `spring-addons-starter-rest` now evicts a rejected authorized client from the store which actually holds it. The `RestClient` interceptor used to remove it from the `OAuth2AuthorizedClientRepository` only, which is a no-op for a registration whose tokens are kept in the `(Reactive)OAuth2AuthorizedClientService` (`client_credentials`, typically): the rejected token was sent again until it expired. A new `SpringAddons(Servlet|Server)AuthorizationFailureHandlerSupport` builds a handler removing the authorized client from every store in the context, rather than guessing which one backs a given registration. The repository removal is skipped when the request was issued outside of a request context (a scheduled task, for instance), where there is no HTTP session to remove it from.
- `spring-addons-starter-rest` now also applies that failure handler to the `WebClient` exchange filter function, in both servlet and reactive applications. It had none, so a rejected token was never evicted. `SpringAddons(Servlet|Server)WebClientSupport.registrationExchangeFilterFunction` takes an optional handler in a new overload, the existing signatures are unchanged.
- The servlet `OAuth2AuthorizedClientManager` auto-configured by `spring-addons-starter-oidc` now uses the `OAuth2AuthorizationFailureHandler` bean in the context, as its reactive counterpart already did.

As usual, all of these beans are `@ConditionalOnMissingBean`: exposing your own `(Reactive)OAuth2AuthorizedClientManager`, `(Server)OAuth2AuthorizedClientRepository` or `(Reactive)OAuth2AuthorizationFailureHandler` opts out.
...
- [gh-305](https://github.com/ch4mpy/spring-addons/issues/305) Back `@ImportHttpServices` HTTP Service groups with an already auto-configured REST client, so that the group's `@HttpExchange` proxies share its base URL, headers, authorization and request factory / connector. See [spring-addons-starter-rest README](https://ch4mpy.github.io/spring-addons/rest/usage/#backing-importhttpservices-groups-with-an-auto-configured-client) for details.
- Follow-up to gh-305: a group backed by `com.c4-soft.springaddons.rest.group.<name>` now keeps honoring `spring.http.serviceclient.<name>.*` (Spring Boot's own per-group connect-timeout, read-timeout, redirects, cookie-handling and SSL bundle) as a fallback, instead of it being silently discarded by the referenced client-id's request factory / connector.
- Support the `ReactorClientHttpRequestFactoryBuilder` and `SimpleClientHttpRequestFactoryBuilder` implementations auto-configured by Spring Boot 4, in addition to HttpComponents, JDK and Jetty. A `RestMisconfigurationException` is still thrown for any other builder type.
...
### `9.0.2-M1`
- Add SSL bundle auto-configuration for `RestClient` and `WebClient`. See [the manual](https://ch4mpy.github.io/spring-addons/rest/ssl-bundles/) for details.
...
- Make the `RestClient`/`WebClient` used by the `(Reactive)OAuth2AuthorizedClientProvider` [easily configurable](https://ch4mpy.github.io/spring-addons/oidc/faq/#how-to-configure-the-authorized-client-manager-to-go-through-an-http-proxy)
...
- `OpenidProviderPropertiesResolver` makes multi-tenancy much simpler to implement, including in "dynamic" scenarios (see [spring-addons-starter-oidc README](https://ch4mpy.github.io/spring-addons/oidc/resource-server/#multi-tenancy))
