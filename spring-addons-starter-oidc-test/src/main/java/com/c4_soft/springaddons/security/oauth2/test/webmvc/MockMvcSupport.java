 ...
 * <ul>
 * <li>auto sets "Accept" and "Content-Type" headers according
 * {@code com.c4-soft.springaddons.test.web.default-media-type} and
 * {@code com.c4-soft.springaddons.test.web.default-charset} to test properties, defaulted to
 * {@code application/json} and {@code utf-8}</li>
 * <li>serializes request body according to Content-type using <b>registered message
 * converters</b></li>
 * <li>provides with shortcuts to issue requests in basic but most common cases (no fancy headers,
...
public class MockMvcSupport {
   ...
   * @param serializationHelper used to serialize payloads to requested {@code Content-type} using
   *        Spring registered message converters
   * @param mockMvcProperties default values for media-type, charset and https usage
   */
  public MockMvcSupport(MockMvc mockMvc, SerializationHelper serializationHelper,
   ...
   * on which issue simple requests in one step.
   *
   * @param accept should be non-empty when issuing response with body (GET, POST, OPTION), none
   *        otherwise
  ...
  /**
   * Factory providing with a request builder to issue a GET request (with Accept header defaulted
   * to what this helper is constructed with).
   *
   * @param urlTemplate API end-point to call
   * @param uriVars values to feed URL template placeholders
   * @return a request builder to be further configured (additional headers, cookies, etc.)
  ...
  /**
   * Shortcut to create a builder for a GET request with minimal headers and submit it (Accept
   * header defaulted to what this helper was constructed with).
   *
   * @param urlTemplate API endpoint to be requested
   * @param uriVars values to replace endpoint placeholders with
   * @return API response to test
  ...
  /* POST */
  /**
   * Factory for a POST request builder containing a body set to payload serialized in given media
   * type (with adequate Content-type header).
   *
  ...
  /**
   * Factory for a POST request builder. Body is pre-set to payload. Both Content-type and Accept
   * headers are set to default media-type.
   *
  ...
  /**
   * Shortcut to issue a POST request with provided payload as body, using given media-type for
   * serialization (and Content-type header).
   *
   ...
   * @return API response to test
   */
  public <T> ResultActions post(T payload, MediaType contentType, Charset charset, MediaType accept,
   ...
   * @return API response to test
   */
  public <T> ResultActions post(T payload, MediaType contentType, MediaType accept,
      String urlTemplate, Object... uriVars) {
    return perform(postRequestBuilder(payload, contentType, accept, urlTemplate, uriVars));
  }

  /**
   * Shortcut to issue a POST request with provided payload as body, using default media-type for
   * serialization (and Content-type header).
   *
   * @param payload POST request body
   * @param urlTemplate API end-point to be called
  ...
  /**
   * Factory for a POST request builder containing a body. Default media-type is used for payload
   * serialization (and Content-type header).
   *
   * @param payload to be serialized as body in contentType format
   * @param urlTemplate API end-point to be requested
   ...
   */
  public <T> ResultActions put(T payload, MediaType contentType, String charset, String urlTemplate,
      Object... uriVars) {
   ...
   * @return API response to be tested
   */
  public <T> ResultActions put(T payload, MediaType contentType, String urlTemplate,
      Object... uriVars) {
   ...
   * @return API response to be tested
   */
  public <T> ResultActions patch(T payload, MediaType contentType, Charset charset,
      String urlTemplate, Object... uriVars) {
   ...
   * @return API response to be tested
   */
  public <T> ResultActions patch(T payload, MediaType contentType, String urlTemplate,
      Object... uriVars) {
    return perform(patchRequestBuilder(payload, contentType, urlTemplate, uriVars));
  }

  /**
   * Shortcut to issue a patch request with Content-type header and a body (using default
   * media-type).
   *
   * @param payload request body
   * @param urlTemplate end-point URL
   ...
   * @param mediaType what format you want payload to be serialized to (corresponding
   *        HttpMessageConverter must be registered)
  ...
  /**
   * @param postProcessor request post-processor to be added to the list of those applied before
