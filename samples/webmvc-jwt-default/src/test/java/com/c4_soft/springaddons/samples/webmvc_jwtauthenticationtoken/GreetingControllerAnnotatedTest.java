...
@AutoConfigureAddonsWebmvcResourceServerSecurity // If your web-security depends on it, setup
...
class GreetingControllerAnnotatedTest {
  ...
  void givenUserHasMockedAuthenticated_whenGetGreet_thenOk() throws Exception {
    api.get("/greet").andExpect(
        content().string("Hello user! You are granted with [ROLE_AUTHORIZED_PERSONNEL]."));
  }

  @Test
  @WithMockAuthentication(authType = JwtAuthenticationToken.class, principalType = Jwt.class,
      name = "Ch4mpy", authorities = "ROLE_AUTHORIZED_PERSONNEL")
  void givenUserIsMockedAsCh4mpy_whenGetGreet_thenOk() throws Exception {
    api.get("/greet").andExpect(
        content().string("Hello Ch4mpy! You are granted with [ROLE_AUTHORIZED_PERSONNEL]."));
  }

  @ParameterizedTest
  @AuthenticationSource({
      @WithMockAuthentication(authType = JwtAuthenticationToken.class, principalType = Jwt.class,
          name = "ch4mp", authorities = "ROLE_AUTHORIZED_PERSONNEL"),
      @WithMockAuthentication(authType = JwtAuthenticationToken.class, principalType = Jwt.class,
          name = "tonton-pirate", authorities = {"UNCLE", "SKIPPER"})})
  void givenUserIsCh4mpy_whenGetGreet_thenOk(@ParameterizedAuthentication Authentication auth)
      throws Exception {
    api.get("/greet").andExpect(content().string(
  ...
  void givenUserIsNotGrantedWithAuthorizedPersonnel_whenGetSecuredMethod_thenForbidden()
    ...
    TestSecurityContextHolder.setAuthentication(mock(JwtAuthenticationToken.class));
  ...
  void givenUserIsAuthenticatedWithJsonClaims_whenGetClaims_thenOk(
      @ParameterizedAuthentication Authentication auth) throws Exception {
    api.get("/greet").andExpect(status().isOk()).andExpect(content().string(
        "Hello %s! You are granted with %s.".formatted(auth.getName(), auth.getAuthorities())));
  }
