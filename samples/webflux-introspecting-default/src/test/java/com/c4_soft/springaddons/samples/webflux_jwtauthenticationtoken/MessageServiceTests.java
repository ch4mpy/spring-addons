...
class MessageServiceTests {
  ...
  void givenUserIsAuthenticated_whenGetGreet_thenReturnsGreeting(
    ...
    assertThat(messageService.greet((BearerTokenAuthentication) auth).block()).isEqualTo(
  ...
  void givenUserIsAuthenticatedWithMockedAuthenticationAndGrantedWithAuthorizedPersonnel_whenGetSecret_thenReturnsSecret() {
    final var auth =
        (BearerTokenAuthentication) TestSecurityContextHolder.getContext().getAuthentication();
    when(auth.getTokenAttributes())
        .thenReturn(Map.of(StandardClaimNames.PREFERRED_USERNAME, "ch4mpy"));

    assertThat(messageService.getSecret().block()).isEqualTo("incredible");
  }

  @ParameterizedTest
  @AuthenticationSource({@WithMockAuthentication(authType = BearerTokenAuthentication.class,
      principalType = OAuth2AccessToken.class, name = "Ch4mpy", authorities = {"NICE", "AUTHOR"}),
      @WithMockAuthentication(authType = BearerTokenAuthentication.class,
          principalType = OAuth2AccessToken.class, name = "Tonton-Pirate",
