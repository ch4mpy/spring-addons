...
public class GreetingControllerFluentApiTest {
  ...
  public void setUp() {
    when(messageService.greet(any())).thenAnswer(invocation -> {
      ...
      return Mono.just(String.format("Hello %s! You are granted with %s.",
          auth.getTokenAttributes().get(StandardClaimNames.PREFERRED_USERNAME),
          auth.getAuthorities()));
    });
  ...
  void givenRequestIsAnonymous_whenGetGreet_thenUnauthorized() throws Exception {
    api.mutateWith(SecurityMockServerConfigurers.mockAuthentication(ANONYMOUS)).get()
        .uri("https://localhost/greet").exchange().expectStatus().isUnauthorized();
  }

  @Test
  void givenUserIsCh4mpy_whenGetGreet_thenOk() throws Exception {
    api.mutateWith(ch4mpy()).get().uri("https://localhost/greet").exchange()
  ...
  void givenUserIsNotGrantedWithAuthorizedPersonnel_whenGetSecuredRoute_thenForbidden()
      throws Exception {
    api.mutateWith(mockOpaqueToken()).get().uri("https://localhost/secured-route").exchange()
  ...
  void givenUserIsNotGrantedWithAuthorizedPersonnel_whenGetSecuredMethod_thenForbidden()
      throws Exception {
    api.mutateWith(mockOpaqueToken()).get().uri("https://localhost/secured-method").exchange()
        .expectStatus().isForbidden();
  }

  @Test
  void givenUserIsGrantedWithAuthorizedPersonnel_whenGetSecuredRoute_thenOk() throws Exception {
    api.mutateWith(ch4mpy()).get().uri("https://localhost/secured-route").exchange().expectStatus()
        .isOk();
  }

  @Test
  void givenUserIsGrantedWithAuthorizedPersonnel_whenGetSecuredMethod_thenOk() throws Exception {
    api.mutateWith(ch4mpy()).get().uri("https://localhost/secured-method").exchange().expectStatus()
        .isOk();
  }
