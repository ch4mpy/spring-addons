...
class GreetingControllerFluentApiTest {
  ...
  void givenRequestIsAnonymous_whenGetGreet_thenUnauthorized() throws Exception {
    api.with(SecurityMockMvcRequestPostProcessors.anonymous()).get("/greet")
        .andExpect(status().isUnauthorized());
  }

  @Test
  void givenUserIsAuthenticated_whenGetGreet_thenOk() throws Exception {
    api.with(mockAuthentication(JwtAuthenticationToken.class, mock(Jwt.class)).name("user"))
        .get("/greet").andExpect(content().string("Hello user! You are granted with []."));
  }

  @Test
  void givenUserIsCh4mpy_whenGetGreet_thenOk() throws Exception {
    api.with(ch4mpy()).get("/greet").andExpect(
  ...
  void givenUserIsNotGrantedWithAuthorizedPersonnel_whenGetSecuredRoute_thenForbidden()
      throws Exception {
    api.with(mockAuthentication(JwtAuthenticationToken.class, mock(Jwt.class)))
  ...
  private MockAuthenticationRequestPostProcessor<JwtAuthenticationToken> ch4mpy() {
    return mockAuthentication(JwtAuthenticationToken.class, mock(Jwt.class)).name("Ch4mpy")
