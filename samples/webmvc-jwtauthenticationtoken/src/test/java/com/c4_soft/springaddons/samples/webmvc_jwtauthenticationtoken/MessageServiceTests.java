...
class MessageServiceTests {
	...
	void whenAuthenticatedThenGreetReurns() {
		final JwtAuthenticationToken auth = mock(JwtAuthenticationToken.class);
		...
		when(auth.getAuthorities()).thenReturn(Arrays.asList(new SimpleGrantedAuthority("ROLE_AUTHORIZED_PERSONNEL")));
	...
	void whenMockedWithAuthorizedPersonelThenGetSecretReturns() {
		final JwtAuthenticationToken auth = (JwtAuthenticationToken) TestSecurityContextHolder.getContext().getAuthentication();
		final Map<String, Object> claims = new HashMap<>();
		claims.put(StandardClaimNames.PREFERRED_USERNAME, "ch4mpy");
		when(auth.getTokenAttributes()).thenReturn(claims);
