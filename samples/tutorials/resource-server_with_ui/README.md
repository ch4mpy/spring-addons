...
This one is tricky: very few "OIDC" authorization-servers follow the standard when it comes to logout. In the three we use in this tutorial, only Keycloak implements strictly the standard.
...
- a composite handler capable of switching between the Spring standard OIDC logout handler and this configurable handler, depending on the authorized client to logout
