...
public class SerializationHelper {
   ...
   * converters. Each acceptable converter
   * ({@link org.springframework.http.converter.HttpMessageConverter#canWrite(Class, MediaType)
   * converter.canWrite(payload.getClass(), mediaType)} return true) is tried until one actually
   * succeeds at serializing.
   *
   * @param <T> payload type
   * @param payload request body to serialize
   * @param mediaType expected body media-type
   * @return serialized payload in JSON, XML, or whatever media-type an HttpMessageConverter is
   *         registered for
   */
  public <T> ByteArrayHttpOutputMessage outputMessage(final T payload, final MediaType mediaType) {
    ...
    final var relevantConverters =
        StreamSupport.stream(messageConverters.getObject().spliterator(), false)
    ...
    if (!isConverted) {
      throw new ConversionFailedException(
          "Could not convert " + payload.getClass() + " to " + mediaType.toString());
    }
