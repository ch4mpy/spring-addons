...
- [2. Usage](#usage)
  ...
  - [2.4. Using with `@ImportHttpServices`](#import-http-services)
...
### <a name="import-http-services" />2.4. Using with `@ImportHttpServices`
Since Spring Boot 4, the `@HttpExchange` proxies can be generated by the framework. Each `RestClient` or `WebClient` bean auto-configured by `spring-addons-starter-rest` can be used bound to a proxy group.
