...
`client-http-request-factory-impl` can also be forced to `JDK`, `HTTP_COMPONENTS` or `JETTY`: in that case, the context builder is ignored and a fresh instance is always built with the selected implementation, exactly as before Spring Boot 3.4 introduced auto-configured HTTP client builders.
