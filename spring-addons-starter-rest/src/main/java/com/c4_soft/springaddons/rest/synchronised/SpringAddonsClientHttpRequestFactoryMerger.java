...
class SpringAddonsClientHttpRequestFactoryMerger {
  ...
  static ClientHttpRequestFactory merge(String clientId, @Nullable ProxySupport proxySupport,
    ...
    if (!needsCustomization) {
      ...
      final var settings = contextSettings.orElseGet(ClientHttpRequestFactorySettings::defaults);
    ...
    var settings = contextSettings.orElseGet(ClientHttpRequestFactorySettings::defaults);
  ...
  private static ClientHttpRequestFactoryBuilder<?> customize(String clientId,
    ...
    if (builder instanceof JdkClientHttpRequestFactoryBuilder jdk) {
      // Boot 3.5 has no JdkClientHttpRequestFactoryBuilder.withExecutor(): set it on the JDK builder
      return jdk.withHttpClientCustomizer(hcb -> {
        virtualThreadsExecutor.ifPresent(hcb::executor);
        if (proxyActive) {
