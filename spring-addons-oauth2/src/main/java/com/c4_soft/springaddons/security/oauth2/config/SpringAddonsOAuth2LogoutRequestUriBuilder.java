...
public class SpringAddonsOAuth2LogoutRequestUriBuilder implements LogoutRequestUriBuilder {
    ...
    public String getLogoutRequestUri(OAuth2AuthorizedClient authorizedClient, String idToken,
        ...
        final var logoutProps = clientProps.getLogoutProperties(clientRegistration.getRegistrationId());
    ...
    @Override
    public String getLogoutRequestUri(OAuth2AuthorizedClient authorizedClient, String idToken) {
        return getLogoutRequestUri(authorizedClient, idToken, clientProps.getPostLogoutRedirectUri());
    }

    public Optional<URI> getLogoutEndpointUri(Optional<OAuth2LogoutProperties> logoutProps,
            ClientRegistration clientRegistration) {
        if (logoutProps.isPresent()) {
            return Optional.ofNullable(logoutProps.get().getUri());
        }
        final var oidcConfig = clientRegistration.getProviderDetails()
                .getConfigurationMetadata();
        return Optional.ofNullable(oidcConfig.get(OIDC_RP_INITIATED_LOGOUT_CONFIGURATION_ENTRY)).map(Object::toString)
                .map(URI::create);
    }
    ...
    static final class MisconfiguredProviderException extends RuntimeException {
        public MisconfiguredProviderException(String clientRegistrationId) {
            super("OAuth2 client registration for %s RP-Initiated Logout is missconfigured: it is neither OIDC complient nor difiend in spring-addons properties"
