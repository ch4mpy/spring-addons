...
public class HttpServletRequestSupport {
	 ...
	 */
	public static String getUniqueHeader(String headerName) throws MissingHeaderException, MultiValuedHeaderException {
		final var headers = getNonEmptyHeaderValues(headerName);
		if (headers.size() < 1) {
	 ...
	 */
	public static List<String> getNonEmptyHeaderValues(String headerName) {
		return getRequest()
				.map(
						req -> StreamSupport
								.stream(Spliterators.spliteratorUnknownSize(req.getHeaders(headerName).asIterator(), Spliterator.ORDERED), false)
